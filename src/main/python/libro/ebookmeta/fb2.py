...
class Fb2Meta:
    ...
    def get_metadata(self, short_author_name=False):

        meta = Metadata()
        meta.title = xstr(self.get('//fb:description/fb:title-info/fb:book-title/text()'))
        meta.author = self.get_author_list(short_author_name)
        meta.author_sort = []
    ...
    def get_author_list(self, short_author_name=False):

        node_list = self.getall('//fb:description/fb:title-info/fb:author')
        author_list = []
        for node in node_list:
            author = self.get_person(node, short_author_name)
    ...
    def get_person(self, node, short_author_name=False):
